...
% Function that looks at cells currently labeled as putative pyramidal
% neurons that have hgigher than normal values on the red channel,
% indicating that they could be interneurons

% INPUTS:
    % percent = threshold above which pyramidal cells are considered 
    %   potential interneurons  
    % img = structure with reference images
    % img_thresholds = adjust the brightness of each background
...
% OUTPUTS: 
    % 3 images from the red channel at different excitation wavelengths
    %  with candidate interneuron masks plotted 
...
%% GET POTENTIAL INTERNEURONS
% uses check_green_long and check_green_short to find candidates at both
% short and long wavelengths. A cell is a candidate if it is above the
% threshold on either image 
%  
